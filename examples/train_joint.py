...
class Runner(object):
    ...
    def run(self):
        ...
        if args.data_config is not None:
            ...
            print(f"this_task_info: {this_task_info}")
        else:
        ...
        # Create data loaders
        if args.setting == 1:
            training_order = ['market1501', 'ltcc', 'llcm']
        elif args.setting == 2:
            training_order = ['llcm', 'ltcc', 'market1501']
        ...
        print(f"build train_loader for task: {this_task_info.task_name}")
        if args.aug:
        ...
        model_path = 'logs/pretrained/pass_vit_base_full.pth'
        if model_path != '':
            model.visual_encoder.load_param(model_path,hw_ratio=2)
        print(msg)

        # continual finetuning 용 코드 블록
        if args.resume:
