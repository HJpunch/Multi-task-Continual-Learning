...
class PASS_Transformer_DualAttn_joint(nn.Module):
    def __init__(self, num_classes, net_config, num_cams=None, this_task_info=None):
        ...
        if net_config.vit_type=='base':
            ...
            self.text_encoder_m = BertForMaskedLM.from_pretrained('bert-base-uncased', config=bert_config)
    ...
    def forward(self, x, instruction, this_task_info=None, label=None, cam_label=None, view_label=None, forzen=False, dkp=False):
        ...
        if self.training:
            ...
            if ('attr' in self.net_config.test_task_type or 'sc' in self.net_config.test_task_type or 't2i' in self.net_config.test_task_type or 'cross' in self.net_config.test_task_type or 'cc' in self.net_config.test_task_type) and 'ctcc' not in self.net_config.test_task_type:
                instruction_text = self.tokenizer(instruction, padding='max_length', max_length=70, return_tensors="pt").to('cuda')
                # extract text features
                text_output = self.text_encoder.bert(instruction_text.input_ids, attention_mask=instruction_text.attention_mask, return_dict=True, mode='text')
                text_embeds = text_output.last_hidden_state
        ...
        if 't2i' in task_name:
            ...
            if not self.training:
                return global_feat, [bio_f, clot_f], torch.cat([bio_f, clot_f], dim=1), text_embeds_s, text_embeds
